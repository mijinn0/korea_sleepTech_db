...
### 스토어드 프로시저 작성: SQL 내의 조건문이나 반복문은 SQL 스크립트 내에서 바로 실행 X, 프,로시저나 트리거 내에서 사용 ###
delimiter $$
...
begin
	...
	if from_balance >= 1000 then
		...
		update accounts
		set balance = balance + 1000
		where account_holder = '류민석';
		
		insert into transaction_log (from_account_id, to_account_id, amount)
		values(
			(select account_id from accounts where account_holder = '이상혁'),
			(select account_id from accounts where account_holder = '류민석'),
			1000
		);
