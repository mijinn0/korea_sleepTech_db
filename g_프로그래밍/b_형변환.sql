...
# cf) 형 변환시 정수형 데이터타입 - signed, unsigned만 사용 가능 (tinyint, smallint 등은 사용 불가!)
# signed: 부호가 있는 정수 (양수/음수 모두 표현 가능)
# unsigned: 부호가 없는 정수

select
	cast(avg(price) as signed) '정수 평균 가능'
from
	buy; # 143
...
select cast('12/12/2022' as Date); -- 형식이 너무 벗어나는 경우 오류 발행 (변환 X)
